...
class Arboles:
    ...
    def clasificacion_turismo(self, database):
        ...
        for x in range(0, 32):
            if float(valores_anuales[8][x]["2018"]) >= 8.0:
                database.actualizar_estado_turismo(x,"* El crecimiento turistico estara muy bajo durante 2019.")
            if float(valores_anuales[8][x]["2018"]) < 8.0 and float(valores_anuales[8][x]["2018"]) > 3.0:
    ...
    def clasificacion_actividad_economica(self, database):
        print("[DEV] ARBOL DE DECISION: clasificacion de actividad economica")